d3jsNode = ->
	...
	link: (scope, element, attrs) ->
		console.log 'd3jsNodeS'
		con = scope.nodesModel
		con = config
		width = 960
		height = 500
		...
		svg = d3.select(attrs.id).append("svg").attr("width", width).attr("height", height)
		d3.json JSON.stringify(config), (error, graph) ->
